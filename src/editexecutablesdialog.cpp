...
EditExecutablesDialog::EditExecutablesDialog(const ExecutablesList& executablesList, const ModList& modList,
                                             Profile* profile, QWidget* parent)
    : TutorableDialog("EditExecutables", parent), ui(new Ui::EditExecutablesDialog), m_CurrentItem(nullptr),
      m_ExecutablesList(executablesList), m_Profile(profile) {
    ui->setupUi(this);
...
EditExecutablesDialog::~EditExecutablesDialog() { delete ui; }
...
void EditExecutablesDialog::saveExecutable() {
    m_ExecutablesList.updateExecutable(
        ui->titleEdit->text(), QDir::fromNativeSeparators(ui->binaryEdit->text()), ui->argumentsEdit->text(),
        ...
        (ui->useAppIconCheckBox->isChecked() ? Executable::UseApplicationIcon : Executable::Flags()) |
            Executable::CustomExecutable);

    if (ui->newFilesModCheckBox->isChecked()) {
        m_Profile->storeSetting("custom_overwrites", ui->titleEdit->text(), ui->newFilesModBox->currentText());
    } else {
...
void EditExecutablesDialog::on_browseButton_clicked() {
    QString binaryName = FileDialogMemory::getOpenFileName("editExecutableBinary", this, tr("Select a binary"),
                                                           QString(), tr("Executable (%1)").arg("*.exe *.bat *.jar"));
    ...
    if (binaryName.endsWith(".jar", Qt::CaseInsensitive)) {
        ...
        { // try to find java automatically
            ...
            if (::FindExecutableW(binaryNameW.c_str(), nullptr, buffer) > reinterpret_cast<HINSTANCE>(32)) {
        ...
        if (binaryPath.isEmpty()) {
            QMessageBox::information(this, tr("Java (32-bit) required"),
                                     tr("MO requires 32-bit java to run this application. If you already have it "
                                        "installed, select javaw.exe "
...
void EditExecutablesDialog::on_browseDirButton_clicked() {
    QString dirName = FileDialogMemory::getExistingDirectory("editExecutableDirectory", this, tr("Select a directory"));
...
void EditExecutablesDialog::on_removeButton_clicked() {
    if (QMessageBox::question(this, tr("Confirm"),
                              tr("Really remove \"%1\" from executables?").arg(ui->titleEdit->text()),
...
bool EditExecutablesDialog::executableChanged() {
    if (m_CurrentItem != nullptr) {
        ...
        return !ui->binaryEdit->text().isEmpty() && !ui->titleEdit->text().isEmpty() && fileInfo.exists() &&
               fileInfo.isFile();
    }
}
...
void EditExecutablesDialog::on_overwriteAppIDBox_toggled(bool checked) { ui->appIDOverwriteEdit->setEnabled(checked); }

void EditExecutablesDialog::on_closeButton_clicked() {
    if (executableChanged()) {
        QMessageBox::StandardButton res = QMessageBox::question(
            this, tr("Save Changes?"), tr("You made changes to the current executable, do you want to save them?"),
...
void EditExecutablesDialog::on_newFilesModCheckBox_toggled(bool checked) { ui->newFilesModBox->setEnabled(checked); }
