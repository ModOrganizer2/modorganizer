...
bool InstanceManager::deleteLocalInstance(const QString& instanceId) const {
    ...
    if (QMessageBox::warning(
            nullptr, QObject::tr("Deleting folder"),
            QObject::tr("I'm about to delete the following folder: \"%1\". Proceed?").arg(instancePath),
            QMessageBox::No | QMessageBox::Yes, QMessageBox::No) == QMessageBox::No) {
        return false;
    }

    if (!MOBase::shellDelete(QStringList(instancePath), true)) {
        qWarning("Failed to shell-delete \"%s\" (errorcode %lu), trying regular delete", qPrintable(instancePath),
                 ::GetLastError());
...
QString InstanceManager::manageInstances(const QStringList& instanceList) const {
    SelectionDialog selection(
        QString("<h3>%1</h3><br>%2")
            ...
            .arg(QObject::tr("Be Carefull! Deleting an Instance will remove all your files for that Instance (mods, "
                             "downloads, profiles, configuration, ...). Custom paths outside of the instance folder "
                             "for downloads, mods, etc. will be left untoched.")),
        nullptr);
    ...
    if (selection.exec() == QDialog::Rejected) {
        ...
        {
            if (QMessageBox::warning(
                    nullptr, QObject::tr("Are you sure?"),
                    QObject::tr("Are you really sure you want to delete the Instance \"%1\" with all its files?")
                        .arg(choice),
...
QString InstanceManager::queryInstanceName(const QStringList& instanceList) const {
    ...
    while (instanceId.isEmpty()) {
        ...
        dialog.setLabelText(
            QObject::tr("Enter a new name or select one from the sugested list (only letters and numbers allowed):"));
        ...
        dialog.setComboBoxItems(
            {"NewName", "Fallout 4", "SkyrimSE", "Skyrim", "Fallout 3", "Fallout NV", "FO4VR", "Oblivion"});
        ...
        if (alreadyExists) {
            ...
            msgBox.setInformativeText(
                QObject::tr("Please choose a different instance name, like: \"%1 1\" .").arg(instanceId));
            msgBox.exec();
            instanceId = "";
        }
    }
...
QString InstanceManager::chooseInstance(const QStringList& instanceList) const {
    enum class Special : uint8_t { NewInstance, Portable, Manage };
    ...
    SelectionDialog selection(
        QString("<h3>%1</h3><br>%2")
            ...
            .arg(QObject::tr("Each Instance is a full set of MO data files (mods, "
                             ...
                             "instances for different games. Instances are stored in Appdata and can be accessed by "
                             "all MO installations. "
    ...
    selection.addChoice(QIcon(":/MO/gui/remove"), QObject::tr("Manage Instances"), QObject::tr("Delete an Instance."),
    ...
    if (choice.type() == QVariant::String) {
        ...
        case Special::NewInstance:
            return queryInstanceName(instanceList);
