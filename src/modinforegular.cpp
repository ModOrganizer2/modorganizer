...
ModInfoRegular::ModInfoRegular(const QDir& path, DirectoryEntry** directoryStructure)
    : ModInfoWithConflictInfo(directoryStructure), m_Name(path.dirName()), m_Path(path.absolutePath()), m_Repository(),
      m_MetaInfoChanged(false), m_EndorsedState(ENDORSED_UNKNOWN), m_NexusBridge() {
    testValid();
    m_CreationTime = QFileInfo(path.absolutePath()).created();
    // read out the meta-file for information
    readMeta();

    connect(&m_NexusBridge, SIGNAL(descriptionAvailable(int, QVariant, QVariant)), this,
            SLOT(nxmDescriptionAvailable(int, QVariant, QVariant)));
    connect(&m_NexusBridge, SIGNAL(endorsementToggled(int, QVariant, QVariant)), this,
            SLOT(nxmEndorsementToggled(int, QVariant, QVariant)));
    connect(&m_NexusBridge, SIGNAL(requestFailed(int, int, QVariant, QString)), this,
            SLOT(nxmRequestFailed(int, int, QVariant, QString)));
}
...
ModInfoRegular::~ModInfoRegular() {
    try {
        ...
        qCritical("failed to save meta information for \"%s\": %s", m_Name.toUtf8().constData(), e.what());
    }
}
...
void ModInfoRegular::nxmRequestFailed(int, int, QVariant userData, const QString& errorMessage) {
    ...
    if (userData.canConvert<int>() && (userData.toInt() == 1)) {
        fullMessage += "\nNexus will reject endorsements within 15 Minutes of a failed attempt, the error message may "
                       "be misleading.";
    }
...
bool ModInfoRegular::setName(const QString& name) {
    ...
    if (m_Name.compare(name, Qt::CaseInsensitive) == 0) {
        ...
        if (!shellRename(modDir.absoluteFilePath(m_Name), modDir.absoluteFilePath(name))) {
            qCritical("failed to rename mod %s (errorcode %d)", qPrintable(name), ::GetLastError());
            return false;
        }
    }
...
QString ModInfoRegular::absolutePath() const { return m_Path; }
...
std::vector<ModInfo::EContent> ModInfoRegular::getContents() const {
    ...
    if (m_LastContentCheck.isNull() || (m_LastContentCheck.secsTo(now) > 60)) {
        ...
        if (dir.entryList(QStringList() << "*.esp"
                                        << "*.esm"
                                        << "*.esl")
                .size() > 0) {
...
QString ModInfoRegular::notes() const { return m_Notes; }

QDateTime ModInfoRegular::creationTime() const { return m_CreationTime; }

QString ModInfoRegular::getNexusDescription() const { return m_NexusDescription; }

QString ModInfoRegular::repository() const { return m_Repository; }

ModInfoRegular::EEndorsedState ModInfoRegular::endorsedState() const { return m_EndorsedState; }

QDateTime ModInfoRegular::getLastNexusQuery() const { return m_LastNexusQuery; }
...
QString ModInfoRegular::getURL() const { return m_URL; }
...
std::vector<QString> ModInfoRegular::getIniTweaks() const {
    ...
    if (numTweaks != 0) {
        qDebug("%d active ini tweaks in %s", numTweaks, QDir::toNativeSeparators(metaFileName).toUtf8().constData());
    }
