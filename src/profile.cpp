...
void Profile::touchFile(QString fileName) {
    ...
    if (!modList.open(QIODevice::ReadWrite)) {
        throw std::runtime_error(
            QObject::tr("failed to create %1").arg(m_Directory.filePath(fileName)).toUtf8().constData());
    }
}

Profile::Profile(const QString& name, IPluginGame const* gamePlugin, bool useDefaultSettings)
    : m_ModListWriter(std::bind(&Profile::doWriteModlist, this)), m_GamePlugin(gamePlugin) {
...
Profile::Profile(const QDir& directory, IPluginGame const* gamePlugin)
    : m_Directory(directory), m_GamePlugin(gamePlugin), m_ModListWriter(std::bind(&Profile::doWriteModlist, this)) {
...
Profile::Profile(const Profile& reference)
    : m_Directory(reference.m_Directory), m_ModListWriter(std::bind(&Profile::doWriteModlist, this)),
...
void Profile::writeModlist() { m_ModListWriter.write(); }

void Profile::writeModlistNow(bool onlyIfPending) { m_ModListWriter.writeImmediately(onlyIfPending); }
...
void Profile::renameModInList(QFile& modList, const QString& oldName, const QString& newName) {
    ...
    if (renamed)
        qDebug("Renamed %d \"%s\" mod to \"%s\" in %s", renamed, qPrintable(oldName), qPrintable(newName),
               qPrintable(modList.fileName()));
}
        ...
        if (modName.size() > 0) {
            ...
            if (modIndex != UINT_MAX) {
                ...
                if ((modIndex < m_ModStatus.size()) && (info->getFixedPriority() == INT_MIN)) {
                    ...
                    qWarning("no mod state for \"%s\" (profile \"%s\")", qPrintable(modName),
                             m_Directory.path().toUtf8().constData());
                    // need to rewrite the modlist to fix this
                    modStatusModified = true;
                }
            } else {
                qDebug("mod \"%s\" (profile \"%s\") not found", qPrintable(modName),
                       m_Directory.path().toUtf8().constData());
                // need to rewrite the modlist to fix this
                modStatusModified = true;
            }
...
std::vector<std::tuple<QString, QString, int>> Profile::getActiveMods() {
    ...
    for (std::vector<unsigned int>::const_iterator iter = m_ModIndexByPriority.begin();
        ...
        if ((*iter != UINT_MAX) && m_ModStatus[*iter].m_Enabled) {
            ...
            result.push_back(
                std::make_tuple(modInfo->internalName(), modInfo->absolutePath(), m_ModStatus[*iter].m_Priority));
        }
    }
...
void Profile::mergeTweak(const QString& tweakName, const QString& tweakedIni) const {
    ...
    DWORD size = ::GetPrivateProfileSectionNamesW(buffer.data(), bufferSize, tweakNameW.c_str());
    ...
    for (std::vector<std::wstring>::iterator iter = sections.begin(); iter != sections.end(); ++iter) {
        // retrieve the names of all keys
        size =
            ::GetPrivateProfileStringW(iter->c_str(), nullptr, nullptr, buffer.data(), bufferSize, tweakNameW.c_str());
        if (size == bufferSize - 2) {
            throw MyException(QString("Buffer too small. Please report this as a bug. "
                                      "For now you might want to split up %1")
                                  .arg(tweakName));
        ...
        for (std::vector<std::wstring>::iterator keyIter = keys.begin(); keyIter != keys.end(); ++keyIter) {
            // TODO this treats everything as strings but how could I differentiate the type?
            ::GetPrivateProfileStringW(iter->c_str(), keyIter->c_str(), nullptr, buffer.data(), bufferSize,
                                       ToWString(tweakName).c_str());
            ::WritePrivateProfileStringW(iter->c_str(), keyIter->c_str(), buffer.data(), tweakedIniW.c_str());
        }
    }
}
...
void Profile::mergeTweaks(ModInfo::Ptr modInfo, const QString& tweakedIni) const {
    ...
    for (std::vector<QString>::iterator iter = iniTweaks.begin(); iter != iniTweaks.end(); ++iter) {
...
bool Profile::localSavesEnabled() const { return m_Directory.exists("saves"); }
...
bool Profile::enableLocalSaves(bool enable) {
    if (enable) {
        ...
        QMessageBox::StandardButton res =
            QMessageBox::question(QApplication::activeModalWidget(), tr("Delete savegames?"),
...
QString Profile::getLoadOrderFileName() const { return QDir::cleanPath(m_Directory.absoluteFilePath("loadorder.txt")); }

QString Profile::getLockedOrderFileName() const {
    return QDir::cleanPath(m_Directory.absoluteFilePath("lockedorder.txt"));
}

QString Profile::getArchivesFileName() const { return QDir::cleanPath(m_Directory.absoluteFilePath("archives.txt")); }

QString Profile::getDeleterFileName() const {
    return QDir::cleanPath(m_Directory.absoluteFilePath("hide_plugins.txt"));
}

QString Profile::getIniFileName() const { return m_Directory.absoluteFilePath(m_GamePlugin->iniFiles()[0]); }
...
QString Profile::absolutePath() const { return QDir::cleanPath(m_Directory.absolutePath()); }
...
void Profile::removeSetting(const QString& section, const QString& name) { m_Settings->remove(section + "/" + name); }