...
void Settings::query(PluginContainer* pluginContainer, QWidget* parent)
{
	...
	tabs.push_back(std::unique_ptr<SettingsTab>(std::make_unique<GeneralTab>(this, dialog)));
	tabs.push_back(std::unique_ptr<SettingsTab>(std::make_unique<PathsTab>(this, dialog)));
	tabs.push_back(std::unique_ptr<SettingsTab>(std::make_unique<DiagnosticsTab>(this, dialog)));
	tabs.push_back(std::unique_ptr<SettingsTab>(std::make_unique<NexusTab>(this, dialog)));
	tabs.push_back(std::unique_ptr<SettingsTab>(std::make_unique<SteamTab>(this, dialog)));
	tabs.push_back(std::unique_ptr<SettingsTab>(std::make_unique<PluginsTab>(this, dialog)));
	tabs.push_back(std::unique_ptr<SettingsTab>(std::make_unique<WorkaroundsTab>(this, dialog)));
	...
	if (dialog.exec() == QDialog::Accepted)
	{
		...
		for (const auto& k : m_Settings.allKeys())
...
Settings::SettingsTab::~SettingsTab() = default;
	...
	, m_pluginBlacklistList(m_dialog.findChild<QListWidget *>("pluginBlacklist"))
{
	...
	for (auto plugin : m_parent->m_Plugins)
	{
		if (handledNames.contains(plugin->name()))
			continue;
		auto listItem = new QListWidgetItem(plugin->name(), m_pluginsList);
		listItem->setData(Qt::UserRole, QVariant::fromValue(static_cast<void*>(plugin)));
	...
	for (const auto& pluginName : m_parent->m_PluginBlacklist)
	{
...
Settings::WorkaroundsTab::WorkaroundsTab(Settings* m_parent,
	...
	: SettingsTab(m_parent, m_dialog)
	  ...
	  , m_enable_archive_parsing_(m_dialog.findChild<QCheckBox *>("enable_archive_parsing"))
{
	m_appIDEdit->setText(m_parent->getSteamAppID());
	const auto mechanismID = m_parent->getLoadMechanism();
	auto index = 0;
	...
	m_enable_archive_parsing_->setChecked(m_parent->enable_archive_parsing());
}
...
void Settings::WorkaroundsTab::update()
{
	...
	m_Settings.setValue("Settings/archive_conflicts", m_enable_archive_parsing_->isChecked());
}
