...
bool Settings::pluginBlacklisted(const QString& fileName) const { return m_PluginBlacklist.contains(fileName); }
...
void Settings::registerPlugin(IPlugin* plugin) {
    ...
    for (const PluginSetting& setting : plugin->settings()) {
        ...
        m_PluginDescriptions[plugin->name()][setting.key] =
            QString("%1 (default: %2)").arg(setting.description).arg(setting.defaultValue.toString());
    }
}
...
bool Settings::automaticLoginEnabled() const { return m_Settings.value("Settings/nexus_login", false).toBool(); }

QString Settings::getSteamAppID() const {
    return m_Settings.value("Settings/app_id", m_GamePlugin->steamAPPId()).toString();
}

bool Settings::usePrereleases() const { return m_Settings.value("Settings/use_prereleases", false).toBool(); }
...
QString Settings::getConfigurablePath(const QString& key, const QString& def, bool resolve) const {
    QString result = QDir::fromNativeSeparators(
        m_Settings.value(QString("settings/") + key, QString("%BASE_DIR%/") + def).toString());
    if (resolve) {
...
QString Settings::getBaseDirectory() const {
    return QDir::fromNativeSeparators(
        m_Settings.value("settings/base_directory", qApp->property("dataPath").toString()).toString());
}
...
QString Settings::getOverwriteDirectory(bool resolve) const {
    return getConfigurablePath("overwrite_directory", ToQString(AppConfig::overwritePath()), resolve);
}
...
bool Settings::getSteamLogin(QString& username, QString& password) const {
    if (m_Settings.contains("Settings/steam_username") && m_Settings.contains("Settings/steam_password")) {
...
bool Settings::compactDownloads() const { return m_Settings.value("Settings/compact_downloads", false).toBool(); }

bool Settings::metaDownloads() const { return m_Settings.value("Settings/meta_downloads", false).toBool(); }

bool Settings::offlineMode() const { return m_Settings.value("Settings/offline_mode", false).toBool(); }
...
int Settings::crashDumpsMax() const { return m_Settings.value("Settings/crash_dumps_max", 5).toInt(); }
...
LoadMechanism::EMechanism Settings::getLoadMechanism() const {
    ...
    case LoadMechanism::LOAD_SCRIPTEXTENDER:
        return LoadMechanism::LOAD_SCRIPTEXTENDER;
    case LoadMechanism::LOAD_PROXYDLL:
        return LoadMechanism::LOAD_PROXYDLL;
    }
    throw std::runtime_error("invalid load mechanism");
}

void Settings::setupLoadMechanism() { m_LoadMechanism.activate(getLoadMechanism()); }

bool Settings::useProxy() const { return m_Settings.value("Settings/use_proxy", false).toBool(); }

bool Settings::displayForeign() const { return m_Settings.value("Settings/display_foreign", true).toBool(); }

void Settings::setMotDHash(uint hash) { m_Settings.setValue("motd_hash", hash); }

uint Settings::getMotDHash() const { return m_Settings.value("motd_hash", 0).toUInt(); }
        ...
        if (exp.exactMatch(file)) {
            ...
            QString languageString =
                QString("%1 (%2)")
                    ...
                    .arg(locale.nativeCountryName()); // QLocale::languageToString(locale.language());
...
void Settings::addStyles(QComboBox* styleBox) {
    ...
    QDirIterator langIter(QCoreApplication::applicationDirPath() + "/" + ToQString(AppConfig::stylesheetsPath()),
...
void Settings::resetDialogs() { QuestionBoxMemory::resetDialogs(); }
...
Settings::SettingsTab::SettingsTab(Settings* m_parent, SettingsDialog& m_dialog)
    : m_parent(m_parent), m_Settings(m_parent->m_Settings), m_dialog(m_dialog) {}

Settings::SettingsTab::~SettingsTab() {}

Settings::GeneralTab::GeneralTab(Settings* m_parent, SettingsDialog& m_dialog)
    : Settings::SettingsTab(m_parent, m_dialog), m_languageBox(m_dialog.findChild<QComboBox*>("languageBox")),
...
Settings::PathsTab::PathsTab(Settings* parent, SettingsDialog& dialog)
    : SettingsTab(parent, dialog), m_baseDirEdit(m_dialog.findChild<QLineEdit*>("baseDirEdit")),
    ...
    for (const auto& dir : {std::make_pair(m_downloadDirEdit, m_parent->getDownloadDirectory(false)),
...
void Settings::PathsTab::update() {
    ...
    if (QFileInfo(m_baseDirEdit->text()) != QFileInfo(qApp->property("dataPath").toString())) {
...
Settings::DiagnosticsTab::DiagnosticsTab(Settings* m_parent, SettingsDialog& m_dialog)
    : Settings::SettingsTab(m_parent, m_dialog), m_logLevelBox(m_dialog.findChild<QComboBox*>("logLevelBox")),
...
Settings::NexusTab::NexusTab(Settings* parent, SettingsDialog& dialog)
    : Settings::SettingsTab(parent, dialog), m_loginCheckBox(dialog.findChild<QCheckBox*>("loginCheckBox")),
      m_usernameEdit(dialog.findChild<QLineEdit*>("usernameEdit")),
      m_passwordEdit(dialog.findChild<QLineEdit*>("passwordEdit")),
      m_offlineBox(dialog.findChild<QCheckBox*>("offlineBox")), m_proxyBox(dialog.findChild<QCheckBox*>("proxyBox")),
      m_knownServersList(dialog.findChild<QListWidget*>("knownServersList")),
      m_preferredServersList(dialog.findChild<QListWidget*>("preferredServersList")) {
...
Settings::SteamTab::SteamTab(Settings* m_parent, SettingsDialog& m_dialog)
    : Settings::SettingsTab(m_parent, m_dialog), m_steamUserEdit(m_dialog.findChild<QLineEdit*>("steamUserEdit")),
...
Settings::PluginsTab::PluginsTab(Settings* m_parent, SettingsDialog& m_dialog)
    : Settings::SettingsTab(m_parent, m_dialog), m_pluginsList(m_dialog.findChild<QListWidget*>("pluginsList")),
...
void Settings::PluginsTab::update() {
    ...
    for (auto iterPlugins = m_parent->m_PluginSettings.begin(); iterPlugins != m_parent->m_PluginSettings.end();
         ++iterPlugins) {
...
Settings::WorkaroundsTab::WorkaroundsTab(Settings* m_parent, SettingsDialog& m_dialog)
    : Settings::SettingsTab(m_parent, m_dialog), m_appIDEdit(m_dialog.findChild<QLineEdit*>("appIDEdit")),
